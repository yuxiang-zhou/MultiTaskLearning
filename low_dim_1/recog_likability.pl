...
our %classif_opt = (
    "reptree" => "-P <!#sss> -S <!#seed> -I <!#nit> -W weka.classifiers.trees.REPTree -- -M 2 -V 0.0010 -N 3 -S 1 -L 25 -P",
    "svr"     =>  "-C <!#complexity> -N 0 -I \"weka.classifiers.functions.supportVector.RegSMOImproved -L 0.0010 -W 1 -P 1.0E-12 -T 0.0010 -V\" -K \"weka.classifiers.functions.supportVector.PolyKernel -C 250007 -E 1.0\"",
    "svm"     => " -M -C <!#complexity> -L 0.0010 -P 1.0E-12 -N 0 -V -1 -W 1 -K \"weka.classifiers.functions.supportVector.PolyKernel -C 250007 -E 1.0\""
);
my %cmdline_classif_params = (
    "sss"        => 0.05,
    "seed"       => 1,
    "nit"        => 500,
    "complexity" => 1.0
...
foreach (@ARGV) {
    ...
    elsif (my ($v) = $_ =~ /^--classif-(.+)=(.+)$/) {
        $cmdline_classif_params{$1} = $2;
        #print "option: $1 = $2\n";
    }
...
my $classif_options = $classif_opt{$classifier};
while (my ($k, $v) = each(%cmdline_classif_params)) {
    $classif_options =~ s/<!#$k>/$v/g;
}

#print "options: $classif_options\n";
#exit;
...
$train_arff    = preprocess($train_arff, [1,796,797,798,799,800,801,802,803,804]);
$test_arff     = preprocess($test_arff, [1,796,797,798,799,800,801,802,803,804]);
...
my $model_name = $classif_class{$classifier} . $classif_options;
$model_name =~ s/[^\w\.\-]//g;
...
if (!-s $model_file || $overwrite) {
    #my $cmd = "$wekacmd $classif_class{$classifier}  -t $train_arff -v -o -no-cv -d $model_file $classif_options";
    my $cmd = "$wekacmd $classif_class{$classifier}  -t $train_arff -v -no-cv -d $model_file $classif_options > $model_file.txt";
    print "-> $cmd\n" unless ($silent);
    system($cmd);
}
...
if ($silent) {
    ...
    my ($ua) = $ua_tmp =~ /([\d\.]+)/;
    my $wa_tmp = `grep '^WAR' $result_file`;
    my ($wa) = $wa_tmp =~ /([\d\.]+)/;
    print "$ua\t$wa\t$auc\n";
}
else {
